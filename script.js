...
let previousScore = 0;
let tileElements = {}; // для анимации движения
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    function posToCoord(r, c) { return {x:c*75, y:r*75}; } // 75px + gap 10px
    function key(r,c){ return `${r}-${c}`; }

    function renderGrid() {
        // удаляем старые плитки
        for(const k in tileElements) gridContainer.removeChild(tileElements[k]);
        tileElements = {};
        ...
        for(let r=0;r<GRID_SIZE;r++){
            for(let c=0;c<GRID_SIZE;c++){
                ...
                tile.className = "tile tile-"+value+" pop";
                tile.textContent = value;

                const coord = posToCoord(r,c);
                tile.style.transform = `translate(${coord.x}px, ${coord.y}px)`;

                gridContainer.appendChild(tile);
                tileElements[key(r,c)] = tile;
            }
        }
    ...
    function addStartTiles(){ for(let i=0;i<2;i++) addRandomTile(); }
    ...
    function performMove(direction){
        ...
        else if(direction==="right"){rotateGrid(2); for(let r=0;r<GRID_SIZE;r++) grid[r]=moveRowLeft(grid[r]); rotateGrid(2);}
        else if(direction==="up"){rotateGrid(3); for(let r=0;r<GRID_SIZE;r++) grid[r]=moveRowLeft(grid[r]); rotateGrid(1);}
        else if(direction==="down"){rotateGrid(1); for(let r=0;r<GRID_SIZE;r++) grid[r]=moveRowLeft(grid[r]); rotateGrid(3);}
    ...
    function move(direction){
        saveState();
        if(!performMove(direction)) return;
        addRandomTile();
        renderGrid();
        if(!canMove()) setTimeout(()=>gameOverModal.classList.remove("hidden"),80);
    }
    ...
    gridContainer.addEventListener("touchstart",e=>{ startX=e.touches[0].clientX; startY=e.touches[0].clientY;},{passive:true});
